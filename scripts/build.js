...
const buildTargets = ['el-view']; // now only build el-view package dir
...
async function build(target, targetVersion) {
  ...
  console.log('beforen main pkg');
  const mainPkg = require(path.resolve("package.json"));

  const pkgDir = path.resolve(`el-view`);
  console.log('pkg dir is', pkgDir);
  ...
  try {
    ...
    if (buildTypes && pkg.types) {
      ...
      if (result.succeeded) {
        console.log('api extract succes');
        ...
        if (pkg.buildOptions && pkg.buildOptions.dts) {
          ...
          console.log('targe dts path', dtsPath);
      ...
      await fs.move(`${pkgDir}/dist`, 'dist');
    }
  } finally {
    // await restorePkg();
    await packageRestore();
...
const resolvePkgDir = target => path.resolve(__dirname, 'el-view');

function checkSize(target) {
  const pkgDir = path.resolve(__dirname, 'el-view');
